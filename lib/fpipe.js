...
var uncaughtExceptionTrap = function(err) {
			logsink.emit('uncaughtException', err);
		};
		...
		_middleware: {
			...
			set: function(value) { 
				if (value && !Array.isArray(value)) {
					throw new TypeError('[object] _middleware must be an Array!');
				} 
				series = value; 
			},
			enumerable: true
...
Object.defineProperties(Pipe.prototype, {
	...
	use: { 
		value: function(middleware) {
			...
			var copy = this._middleware.slice(0);
			copy.push(middleware);
			this._middleware = copy; 
			return this;
		},
		enumerable: true
	},
	...
	clone: {
		value: function(source) {
			...
			result._middleware = this._middleware;
